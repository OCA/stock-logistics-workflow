...
class StockMove(models.Model):
    ...
    def _previous_instant_date(self):
        ...
        self.ensure_one()
        date_with_delta = fields.Datetime.from_string(self.date) - timedelta(seconds=1)
    ...
    def get_average_price(self, previous_qty, previous_price):
        if self.quantity_done == 0.0:
            ...
            price = (
                (previous_qty * previous_price) + (self.quantity_done * self.price_unit)
    ...
    def cost_price_avco_sync(self):
        ...
        for move in self.sorted("date"):
            if (
                move.id in procesed_moves
                or move.state != "done"
                or not move._is_in()
                or move.product_id.cost_method != "average"
            ):
                continue
            ...
            for af_move in affected_moves:
                if af_move._is_in():
                    ...
                    af_move.write(
                        {"price_unit": -price, "value": -price * af_move.quantity_done,}
                    )
                    previous_qty -= af_move.quantity_done
            if float_compare(
