...
class TestLockingUnlocking(TestStockCommon):
    ...
    def test_lock(self):
        ...
        self.assertTrue(
            self.lot.locked, "The lot should be locked when the button is pressed"
        )
        self.assertTrue(
        ...
        for move in self.picking_out.move_lines:
            self.assertNotEqual(
                ...
                "Reserved lots: %s"
                % [(q.id, q.lot_id.name) for q in move.reserved_quant_ids],
            )
    ...
    def test_unlock(self):
        ...
        self.assertFalse(
            self.lot.locked, "The lot should be unlocked when the button is pressed"
        )
        self.assertFalse(
            all([quant.locked for quant in self.lot.quant_ids]),
        ...
        for move in self.picking_out.move_lines:
            self.assertEqual(
                move.state, "assigned", "The stock move should be assigned"
            )
    ...
    def test_lock_unreserve(self):
        ...
        domain = [("lot_id", "=", self.lot.id), ("reservation_id", "!=", False)]
    ...
    def test_category_locked(self):
        ...
        lot1 = self.LotObj.create(
            {"name": "Lot in locked category", "product_id": self.productA.id}
        )
        self.assertTrue(lot1.locked, "Category demands to lock new lots")

    def test_wizard(self):
        wizard = self.env["wiz.lock.lot"].with_context(active_ids=[self.lot.id])
