...
class StockWarehouse(orm.Model):
    ...
    _columns = {
        ...
        'transit_in_type_id': fields.many2one('stock.picking.type', 'In Transit Type'),
        'transit_out_type_id': fields.many2one('stock.picking.type', 'Out Transit Type'),
        'wh_transit_in_loc_id': fields.many2one('stock.location', 'Incoming Transit'),
        'wh_transit_out_loc_id': fields.many2one('stock.location', 'Outgoing Transit'),
        }

    def create(self, cr, uid, vals, context=None):
        transit_in, transit_out = self._create_transit_locations(cr, uid,
                                                                 vals.get('company_id'),
    ...
    def _create_transit_locations(self, cr, uid, company_id, reception_steps, delivery_steps, context):
        if context is None:
            context = {}
        ctx = context.copy()
        ...
        try:
            out_id = data_obj.get_object_reference(cr, uid, 'stock_route_transit', 'transit_outgoing')[1]
        except:
            out_id = location_obj.create(cr, uid,
