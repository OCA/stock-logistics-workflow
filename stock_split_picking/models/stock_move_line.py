...
class StockMoveLine(models.Model):
    ...
    def _get_original_move_vals(self, qty):
        ...
        new_reserved_qty = self.product_id.uom_id._compute_quantity(
            self.reserved_qty - qty, self.product_uom_id, round=False
        )
        new_reserved_qty = float_round(
            new_reserved_qty, precision_digits=decimal_precision
        )
        return {
            "reserved_uom_qty": new_reserved_qty,
            "qty_done": 0.0,
        }
    ...
    def _split(self, qty):
        ...
        move_line_vals = self.with_context(rounding_method="HALF-UP").copy_data(defaults)
