...
class TestPoPropagate(TransactionCase):
    @classmethod
    def setUpClass(cls):
        def _create_product(name, supplier):
            product_tmpl_model = cls.env["product.template"]
            return (
                product_tmpl_model.with_context(tracking_disable=True)
                .create(
                    {
                        "name": name,
                        "type": "product",
                        ...
                        "seller_ids": [
                            (
                                0,
                                0,
                                {"partner_id": supplier.id, "min_qty": 1, "price": 1.0},
                            )
                        ],
                    }
                )
        ...
        # Create suppliers
        partner_model = cls.env["res.partner"].with_context(tracking_disable=True)
        cls.strings_inc = partner_model.create(
            {"name": "Strings Inc", "company_type": "company"}
        )
        cls.drummers_friend = partner_model.create(
            {"name": "Drummers Friend", "company_type": "company"}
        )
        cls.piano_store = partner_model.create(
            {"name": "Piano store", "company_type": "company"}
        )
    ...
    def test_two_steps(self):
        ...
        internal_picking_moves = internal_picking.move_ids
        internal_picking_products = internal_picking_moves.mapped("product_id")
        ...
        receipt_moves = receipt_picking.move_ids
        ...
        internal_picking_products = internal_picking.move_ids.mapped("product_id")
        ...
        po_drummers.order_line.write({"product_qty": 3.0})
        po_drummers.button_confirm()
        receipt_picking = po_drummers.picking_ids
        self.assertEqual(receipt_picking.location_dest_id, self.input_loc)
        self.assertEqual(len(receipt_picking.move_ids), 1)
        self.assertEqual(receipt_picking.move_ids.product_id, self.drumset)
        self.assertAlmostEqual(receipt_picking.move_ids.product_uom_qty, 3.0)
        internal_move = receipt_picking.move_ids.move_dest_ids
        ...
        self.assertNotIn(self.drumset, internal_picking.move_ids.mapped("product_id"))
        self.env["procurement.group"].run_scheduler()
        self.assertIn(self.drumset, internal_picking.move_ids.mapped("product_id"))
        drums_line = internal_picking.move_ids.filtered(
    ...
    def test_three_steps(self):
        ...
        input_to_qc_picking_products = input_to_qc_picking.move_ids.mapped("product_id")
        qc_to_stock_picking_products = qc_to_stock_picking.move_ids.mapped("product_id")
        ...
        self.assertEqual(input_to_qc_move.picking_id.group_id, po_procurement_group)
        ...
        self.assertEqual(qc_to_stock_move.picking_id.group_id, po_procurement_group)
    ...
    def test_two_warehouses_orderpoints(self):
        ...
        wh_wh2_route = self.env["stock.route"].create(
            {
                "name": "WH > WH2",
        ...
        self.piano.write({"route_ids": [(4, wh_wh2_route.id)]})
        self.piano_op.copy(
            {"name": "OP/%s-2" % self.piano.name, "location_id": wh2.lot_stock_id.id}
        )
        self.env["procurement.group"].run_scheduler()
