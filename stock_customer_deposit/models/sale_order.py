...
class SaleOrder(models.Model):
    ...
    def _action_confirm(self):
        ...
        for order in self.filtered(lambda o: not o.customer_deposit):
            ...
            for product, quants in quants_by_product.items():
                    ...
                    float_compare(
                        ...
                        sum(
                            deposit_lines.filtered(
                                lambda line, product=product: line.product_id.id
                                == product.id
        ...
        for order in self:
            ...
            if order.customer_deposit:
                ...
                if product_lines.filtered(
                    ...
                    raise ValidationError(
                        self.env._(
                            "All lines coming from orders marked as 'Customer depot' "
                ...
                if product_lines.filtered(
                    ...
                    raise ValidationError(
                        self.env._(
                            "You cannot select Customer Deposit route in an order line "
