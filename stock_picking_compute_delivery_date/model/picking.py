...
class StockPickingOut(orm.Model):
    ...
    # Method to post messages generated by each product availability
    # (only one per picking, to avoid a long mail thread)
    def picking_notify(self, cr, uid, messages, context=None):
        for picking_id in messages.keys():
            self.message_post(cr, uid, picking_id,
                              body=messages[picking_id], context=context)
    ...
    def compute_mto_delivery_dates(self, cr, uid, product, context=None):
        ...
        for move_out in self.pool['stock.move'].browse(cr, uid, move_out_ids,
            ...
            if move_in_ids:
                ...
                # Add message on stock picking:
                # - if it does not exist, add the current one
                # - if it does, overwrite it, and keep the "last" one.
                messages[move_out.picking_id.id] = message
        return messages
    ...
    def compute_all_delivery_dates(self, cr, uid, use_new_cursor=True,
        ...
        for product in prod_obj.browse(cr, uid, product_ids, context=context):
            ...
            try:
                ...
                # the message
                messages.update(self.compute_delivery_dates(cr, uid, product,
                                                            context=context))
                if use_new_cursor:
                    cr.commit()
            except:
