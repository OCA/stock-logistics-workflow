...
class CustomerPortal(portal.CustomerPortal):
    def _get_portal_message_author_id(self, record):
        """Author to use when posting a chatter message from the portal side.

        Mirrors the behaviour of the (now removed) ``_message_post_helper``:
        if the current session is a logged in user, the message is posted
        under their name; if it's the public/anonymous user (accessing via
        a share token), the message is posted under the document's own
        partner instead.
        """
        user = request.env.user
        if user._is_public():
            partner = record.partner_id if "partner_id" in record._fields else False
        ...
        if request.env.user.share and access_token:
            ...
            if session_obj_date != today:
                ...
                operation_sudo.with_context(mail_create_nosubscribe=True).message_post(
                    ...
                    author_id=self._get_portal_message_author_id(operation_sudo),
