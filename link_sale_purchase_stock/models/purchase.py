...
class PurchaseOrder(models.Model):
    ...
    @api.depends('order_line.sale_order_id', 'order_line.move_dest_ids.group_id.sale_id')
    def _compute_sale_order_count(self):
        for purchase in self:
            purchase.sale_order_count = len(purchase._get_sale_orders())

    def _get_sale_orders(self):
        return self.order_line.sale_order_id | self.order_line.move_dest_ids.group_id.sale_id
...
class PurchaseOrderLine(models.Model):
    ...
    def _prepare_stock_moves(self, picking):
        ...
        res[0]['location_dest_id'] = self.orderpoint_id and self.orderpoint_id.location_id.id or self.order_id._get_destination_location()
        return res
