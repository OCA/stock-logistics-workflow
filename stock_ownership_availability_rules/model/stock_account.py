...
class stock_quant(osv.osv):
    ...
    """
    Following method is a copy from the official Odoo branch (addons/stock_account/stock_account.py)
    with only change on the condition where judgment on quant's owner is made.
    This is due to the fact that this module makes the owner_id a required field regardless of who own the stock,
    while standard Odoo assumes the field should be blank in case the stock is owned by the company.
    As this overrides the original method, the content may need to be backported from the official branch as necessary.
    """
