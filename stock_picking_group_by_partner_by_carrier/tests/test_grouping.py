...
class TestGroupBy(TestGroupByBase, TransactionCase):
    ...
    def test_backorder_picking_merge(self):
        ...
        move = first(pick.move_ids_without_package)
    ...
    def test_cancelling_sale_order1(self):
        ...
        moves = so1.picking_ids.move_ids_without_package
    ...
    def test_delivery_multi_step(self):
        ...
        self.assertEqual(so1.picking_ids.group_id, so1.picking_ids.move_ids_without_package.group_id)
        # Add a line to so1
        self.assertEqual(len(ships.move_ids_without_package), 2)
        sale_form = Form(so1)
        self._set_line(sale_form, 4)
        sale_form.save()
        self.assertEqual(len(ships.move_ids_without_package), 3)
        # the group is the same on the move lines and picking
        self.assertEqual(len(so1.picking_ids.group_id), 1)
        self.assertEqual(so1.picking_ids.group_id, so1.picking_ids.move_ids_without_package.group_id)
        # Add a line to so2
        ...
        self._set_line(sale_form, 4)
        sale_form.save()
        self.assertEqual(len(ships.move_ids_without_package), 4)
        # the group is the same on the move lines and picking
        self.assertEqual(len(so2.picking_ids.group_id), 1)
        self.assertEqual(so1.picking_ids.group_id, so2.picking_ids.move_ids_without_package.group_id)
    ...
    def test_delivery_multi_step_cancel_so1(self):
        ...
        pick1 = so1.order_line.move_ids_without_package.move_orig_ids.picking_id
        pick2 = so2.order_line.move_ids_without_package.move_orig_ids.picking_id
        so1._action_cancel()
    ...
    def test_delivery_multi_step_cancel_so1_create_so3(self):
        ...
        pick3 = so3.order_line.move_ids_without_package.move_orig_ids.picking_id
    ...
    def test_sale_stock_merge_procurement_group(self):
        ...
        # the group is the same on the move lines and picking
        self.assertEqual(picking.group_id, picking.move_ids_without_package.group_id)
        ...
        move1 = picking.move_ids_without_package.filtered(
    ...
    def test_create_backorder(self):
        ...
        picking.action_assign()
        line = first(picking.move_ids_without_package).move_line_ids
