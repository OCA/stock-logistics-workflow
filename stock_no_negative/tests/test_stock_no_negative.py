...
class TestStockNoNegative(TransactionCase):
    ...
    def test_true_allow_negative_stock_product_with_lot(self):
        ...
        quant_rec = self.env["stock.quant"].create(
            {'product_id': self.product_with_lot.id,
             'location_id': self.location_id.id,
             'quantity': 100,
             'lot_id': self.lot1.id})
        self.stock_picking_with_lot.action_confirm()
        self.stock_picking_with_lot.move_ids.quantity = 200
        with self.assertRaises(UserError):
            self.stock_picking_with_lot.move_ids.picked = True
            self.stock_picking_with_lot._action_done()
        self.stock_picking_with_lot.move_ids.move_line_ids[0].write({'quant_id': quant_rec.id, 'lot_id':self.lot1.id})
        self.stock_picking_with_lot._action_done()
        quant = self.env["stock.quant"].search(
            [
                ("product_id", "=", self.product_with_lot.id),
