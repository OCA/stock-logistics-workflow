...
class TestOperationBatchQuickChange(TransactionCase):
    def setUp(self):
        ...
        self.Wizard = self.env["stock.picking.batch.operation.wizard"]
        ...
        self.batch = self.env["stock.picking.batch"].create({"name": "BP / Test"})
        self.picking.batch_id = self.batch
    ...
    def test_picking_batch_operation_change_location_dest_all(self):
        self.batch.action_assign()
        new_location_dest_id = self.Location.create(
            {
                "name": "New Test Customer Location",
                "location_id": self.batch.picking_ids.location_dest_id.location_id.id,
            }
        )
        wiz = self.Wizard.with_context(
            active_model=self.batch._name,
            active_ids=self.batch.ids,
        ).create({"new_location_dest_id": new_location_dest_id.id, "change_all": True})
        move_lines = self.batch.move_line_ids
        self.assertEqual(wiz.location_dest_id, self.batch.picking_ids.location_dest_id)
        self.assertEqual(wiz.old_location_dest_id, move_lines[:1].location_dest_id)
        wiz.action_apply()
        move_lines = self.batch.mapped("move_line_ids.location_dest_id")
        self.assertEqual(len(move_lines), 1)

    def test_picking_batch_operation_change_location_dest(self):
        new_location_dest_id = self.Location.create(
            {
                "name": "New Test Customer Location",
                "location_id": self.batch.picking_ids.location_dest_id.location_id.id,
        ...
        wiz = self.Wizard.with_context(
            ...
            {
                "old_location_dest_id": self.batch.picking_ids.location_dest_id.id,
