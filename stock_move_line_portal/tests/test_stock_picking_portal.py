...
class TestStockPickingPortal(HttpCase):
    ...
    def _get_picking(self):
        self.env["stock.quant"]._update_available_quantity(
            self.product_a, self.env.ref("stock.stock_location_stock"), 10
        )
    ...
    def test_picking_access_url(self):
        ...
        self.assertEqual(
            picking.access_url,
            f"/my/move_operations/{picking.id}",
    ...
    def test_SO_portal_access_1(self):
        ...
        picking = self._get_picking()
        login = None
        picking_url = "/my/move_operations/%s" % picking.id
    ...
    def test_SO_portal_access_2(self):
        """Check that it is possible to open Move Operations, either using
        the access token or being connected as portal user"""
        ...
        response = self.url_open(
            url=(
                f"/my/move_operations?date_begin={date_begin.strftime('%Y-%m-%d')}"
        ...
        for case in test_cases:
            ...
            if "filterby" in case:
                self.assertEqual(
                    ...
                    [("picking_id.picking_type_id.code", "=", case["filterby"])],
                )
