...
class StockMove(models.Model):
    ...
    def _needs_variable_quantity_backorder(self, precision_digits=None):
        ...
        if not (
            self.move_dest_ids
            and self.picking_type_id.propagate_variable_qty
            and self.picked
            and self.quantity > 0
        ):
            return False
    ...
    def _split_variable_quantity_dest_moves(self, backorder_sources, backorder_moves):
        for source_move, backorder_move in zip(
            ...
            if not source_move.picking_type_id.propagate_variable_qty:
                continue
            dest_moves = (
    ...
    def _get_split_variable_quantity_dest_moves(self, dest_moves, backorder_move):
        ...
        source_moves, dest_moves = self._get_variable_quantity_group(dest_moves)
        source_moves -= backorder_move
        qty_left = sum(source_moves.mapped("quantity"))
    ...
    def _adjust_variable_quantity(self):
        ...
        """
        moves = self.filtered(
            lambda move: move.move_dest_ids
            and move.picking_type_id.propagate_variable_qty
        )
    ...
    def _get_variable_quantity_group(self, dest_moves):
        ...
        while previous_move_count != len(group_moves) or previous_dest_count != len(
            ...
            group_moves |= group_dest_moves.move_orig_ids.filtered(
                lambda move: move.picking_type_id.propagate_variable_qty
