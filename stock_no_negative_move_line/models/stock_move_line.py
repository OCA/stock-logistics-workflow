...
class StockMoveLine(models.Model):
    ...
    def _gather_sanitized(self, location_id=None, lot_id=None, package_id=None):
        location_id = (
            ...
            else self.env["stock.location"].browse(location_id)
        )
        lot_id = (
            self.lot_id
            if lot_id is None
            else self.env["stock.production.lot"].browse(lot_id)
        )
        package_id = (
            self.package_id
            if package_id is None
            else self.env["stock.quant.package"].browse(package_id)
        )
    ...
    def _will_cause_negative(
        self, qty=None, location_id=None, lot_id=None, package_id=None
    ):
        self.ensure_one()
        ...
        if (
            ...
            or self.location_id.usage not in ["internal", "transit"]
            or not is_forbidden
        ):
            return False
        ...
        if (
            float_compare(total_qty_available, qty_done, precision_digits=precision)
            == -1
        ):
            location_id = (
                self.location_id
            ...
            raise ValidationError(
                ...
                % {
                    ...
                    "q_quantity": float_round(
                        total_qty_available - qty_done, precision_digits=precision
                    ),
