...
class StockPicking(models.Model):
    ...
    def action_force_assign_pickings(self):
        ...
        self.message_post(
            ...
            % ", ".join(
                to_unreserve.mapped("picking_id").mapped(
                    lambda x: link_template % (x._name, x.id, x.name)
                )
            )
        )
        to_unreserve.mapped("picking_id").message_post(
            body=_("Unreserved this picking in order to assign %s")
            % ", ".join(self.mapped(lambda x: link_template % (x._name, x.id, x.name)))
        )
    ...
    def _force_assign_allow_partial(self):
        ...
        return tools.misc.str2bool(
            ...
            .sudo()
            .get_param("stock_picking_force_assign.allow_partial"),
            default=False,
        )
