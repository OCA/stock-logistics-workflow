    ...
    def _product_available(self, cr, uid, ids, field_names=None, arg=False, context=None):
        ...
        for f in field_names:
            ...
            if f == 'virtual_available':
                c.update({'states': ('confirmed', 'waiting', 'assigned', 'on_hold',
                                     'on_hold_billing', 'on_hold_paym', 'done'), 'what': ('in', 'out')})
            if f == 'incoming_qty':
                c.update({'states': ('confirmed', 'waiting', 'assigned',
                                     'on_hold', 'on_hold_billing', 'on_hold_paym'), 'what': ('in',)})
            if f == 'outgoing_qty':
                c.update({'states': ('confirmed', 'waiting', 'assigned',
                                     'on_hold', 'on_hold_billing', 'on_hold_paym'), 'what': ('out',)})
