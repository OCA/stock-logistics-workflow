            ...
            if picking:
                ...
                if state:
                    self.pool.get('stock.picking').write(
                        cr, uid, picking.id, {'state': state}, context)
        return []
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if move.state in ('confirmed', 'waiting', 'on_hold_billing', 'on_hold_paym'):
                if move.state in ('on_hold_billing', 'on_hold_paym'):
                    ...
                    # Important: we must pass lock=True to _product_reserve()
                    # to avoid race conditions and double reservations
                    res = self.pool.get('stock.location')._product_reserve(cr, uid, [
                        move.location_id.id], move.product_id.id, move.product_qty, {'uom': move.product_uom.id}, lock=True)
                if res:
                    if move.sale_line_id and move.sale_line_id.order_id.order_policy in ['ship_prepayment', 'wait_prepayment']:
                        if not move.sale_line_id.invoice_lines:
                            self.write(
                                cr, uid, [move.id], {'state': 'on_hold_billing'})
                            pickings[move.picking_id.id] = 1
                            waiting += 1
                        else:
                            for line in move.sale_line_id.invoice_lines:
                                if line.invoice_id and line.invoice_id.state == 'cancel':
                                    ...
                                    self.write(
                                        cr, uid, [move.id], {'state': 'on_hold_paym'})
                                    pickings[move.picking_id.id] = 1
                                    waiting += 1
                                elif line.invoice_id and line.invoice_id.state == 'paid':
                                    self.write(
                                        cr, uid, [move.id], {'state': 'assigned'})
                                    pickings[move.picking_id.id] = 1
                                    waiting += 1
                    ...
                    # the stock outgoing form
                        ...
                        pickings[move.picking_id.id] = 1
                        r = res.pop(0)
                        cr.execute('update stock_move set location_id=%s, product_qty=%s where id=%s', (r[
                                   1], r[0], move.id))

                        while res:
                            r = res.pop(0)
                            move_id = self.copy(
                                cr, uid, move.id, {'product_qty': r[0], 'location_id': r[1]})
                            done.append(move_id)
        if done:
            count += len(done)
...
class stock_picking(osv.osv):
    ...
    def action_on_hold(self, cr, uid, ids, context=None):
        ...
        if len(todo):
            self.pool.get('stock.move').action_on_hold(
                cr, uid, todo, context=context)
        return True
    ...
    def action_hold_to_confirm(self, cr, uid, ids, context=None):
        ...
        todo = self.action_explode(cr, uid, todo, context)
        if len(todo):
            self.pool.get('stock.move').action_hold_to_confirm(
                cr, uid, todo, context=context)
        return True
    ...
    def action_confirm_waiting_bill(self, cr, uid, ids, context=None):
        ...
        """
        self.write(
            cr, uid, ids, {'state': 'on_hold_billing', 'invoice_state': '2binvoiced'})
        todo = []
        for picking in self.browse(cr, uid, ids, context=context):
            for r in picking.move_lines:
        ...
        todo = self.action_explode(cr, uid, todo, context)
        if len(todo):
            self.pool.get('stock.move').action_confirm_waiting_bill(
                cr, uid, todo, context=context)
        return True
    ...
    def action_waiting_bill_to_unpaid(self, cr, uid, ids, context=None):
        ...
        todo = self.action_explode(cr, uid, todo, context)
        if len(todo):
            self.pool.get('stock.move').action_waiting_bill_to_unpaid(
                cr, uid, todo, context=context)
        return True
    ...
    def action_assign(self, cr, uid, ids, *args):
        ...
        for pick in self.browse(cr, uid, ids):
            move_ids = [x.id for x in pick.move_lines if x.state in [
                'confirmed', 'on_hold_billing', 'on_hold_paym']]
            if not move_ids:
                raise osv.except_osv(
                    _('Warning !'), _('Not enough stock, unable to reserve the products.'))
        ...
        for pick in self.browse(cr, uid, ids):
            ...
            for move in pick.move_lines:
                ...
                if (mt == 'direct') and (move.state == 'on_hold_billing') and (move.product_qty):
                    return True
                ok = ok and (
                    move.state in ('cancel', 'done', 'assigned', 'on_hold_paym'))
        return ok
    ...
    def log_picking(self, cr, uid, ids, context=None):
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            message = type_list.get(
                pick.type, _('Document')) + " '" + (pick.name or '?') + "' "
            if pick.min_date:
                msg = _(' for the ') + datetime.strptime(pick.min_date,
            ...
            res = data_obj.get_object_reference(
                cr, uid, 'stock', view_list.get(pick.type, 'view_picking_form'))
    ...
    def already_invoiced(self, cr, uid, ids, pick_invoice, context=None):
        ...
        for pick in picking_ids:
            ...
            if sale_id:
                ...
                amount_total = 0.00
                cr.execute(
                    """SELECT invoice_id FROM sale_order_invoice_rel WHERE order_id = %s """, (sale_id,))
                invoice_ids = map(lambda x: x[0], cr.fetchall())
                invoice_open_paid = invoice_obj.search(
                    cr, uid, [('id', 'in', invoice_ids), ('state', 'in', ['open', 'paid'])])
                ...
                sale_obj.write(cr, uid, [sale_id], {
                               'amount_prepaid': amount_prepaid, 'amount_shipped': amount_total})
        return res
    ...
    def create_advance_line(self, cr, uid, pick_invoice, invoice_done, context=None):
        ...
        for pick in pick_invoice:
            ...
            if prepaid_amount >= amount_shipped:
                ...
                res = account_line_obj.product_id_change(cr, uid, [], 7624, False, 1, partner_id=invoice_id.partner_id.id, fposition_id=invoice_id.fiscal_position.id,
                                                         price_unit=line_amount, address_invoice_id=invoice_id.address_invoice_id.id, currency_id=invoice_id.currency_id.id, context=context)
                account_line_obj = self.pool.get('account.invoice.line')
                vals = res['value']
                vals.update({'invoice_id': invoice_id.id, 'invoice_line_tax_id': [(6, 0, vals[
                            'invoice_line_tax_id'])], 'note': '', 'price_unit': line_amount, 'is_advance': True})
                if vals['price_unit'] <> 0:
                    account_line_obj.create(cr, uid, vals)
            invoice_amount = invoice_id.amount_untaxed
            if sale_id:
                self.pool.get('sale.order').write(
                    cr, uid, [sale_id.id], {'amount_shipped': amount_shipped + invoice_amount})
    ...
    def associate_lines(self, cr, uid, ids, pick_invoice, context):
        ...
        for pick in pick_invoice:
            ...
            for line in move_lines:
                ...
                if invoice_line_ids:
                    move_line_obj.write(
                        cr, uid, [line.id], {'invoice_line_id': invoice_line_ids[0]})
    ...
    def action_invoice_create(self, cr, uid, ids, journal_id=False,
        ...
        if context.get('before_shipping', False):
            ...
            for pick in picking_ids:
                line_ids = [
                    x.id for x in pick.move_lines if x.state == 'on_hold_billing']
                old_lines_ids = [
                    x.id for x in pick.move_lines if x.id not in line_ids]
                if line_ids:
                    wf_service = netsvc.LocalService("workflow")
                    if old_lines_ids:
                        ...
                        self.pool.get('stock.move').write(
                            cr, uid, old_lines_ids, {'picking_id': new_picking}),
                        wf_service.trg_validate(
                            uid, 'stock.picking', new_picking, 'button_confirm', cr)
                    self.pool.get('stock.move').write(
                        cr, uid, line_ids, {'state': 'on_hold_paym'}),
                    todo.append(pick.id)
            ids = todo
        ...
        for id in self.search(cr, uid, [('location_id', 'child_of', ids)]):
            ...
            for r in results:
                amount = self.pool.get('product.uom')._compute_qty(
                    cr, uid, r['product_uom'], r['product_qty'], context.get('uom', False))
                results2 += amount
                total += amount
