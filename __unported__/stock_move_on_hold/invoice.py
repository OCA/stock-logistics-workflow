...
class account_invoice(osv.osv):
    ...
    def confirm_paid(self, cr, uid, ids, context=None):
        ...
        cr.execute(
            """SELECT order_id FROM sale_order_invoice_rel WHERE invoice_id in %s """, (tuple(ids),))
        order_ids = map(lambda x: x[0], cr.fetchall())
        self.pool.get('sale.order').write(cr, uid, order_ids, {})
        cr.execute(
            """SELECT id FROM stock_picking where state = 'on_hold_paym' and sale_id in (SELECT order_id FROM sale_order_invoice_rel WHERE invoice_id in %s) """, (tuple(ids),))
